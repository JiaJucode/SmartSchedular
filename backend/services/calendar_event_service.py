...
def add_calendar_event(title: str, tags: List[str], str_start_datetime: str,
    ...
    start_datetime = datetime.fromisoformat(str_start_datetime).replace(tzinfo=None)
    end_datetime = datetime.fromisoformat(str_end_datetime).replace(tzinfo=None)
    if start_datetime > end_datetime:
        raise ValueError("start_datetime must be before end_datetime")
...
def edit_calendar_event(id: int, title: str, tags: List[str], str_start_datetime: str,
    ...
    unlink_generated_item(id, is_task=False)
